...
function SWEP:MakePaintBlob(paintType)
	if timer.Exists("TA:Player_ShootingPaint" .. self.Owner:EntIndex()) then
		return
	end
	timer.Create(
		"TA:Player_ShootingPaint" .. self.Owner:EntIndex(),
		0.01,
		1,
		function()
		end
	...
	local randSize =
		math.Rand(
		LIB_APERTURECONTINUED.GEL_MINSIZE,
		(LIB_APERTURECONTINUED.GEL_MAXSIZE + LIB_APERTURECONTINUED.GEL_MINSIZE) / 2
	)
	local paint =
		LIB_APERTURECONTINUED:MakePaintBlob(
		paintType,
		ownerShootPos,
		forward * math.max(100, math.min(200, force - 100)) * 8 + VectorRand() * 100 + ownerSpeed,
		randSize
	)
	paint.IsFromPaintGun = true

	if not IsValid(paint) then
		return
	end
	if IsValid(self.Owner) and self.Owner:IsPlayer() then
		paint:SetOwner(self.Owner)
	end
end
...
function SWEP:CalcViewModelView(ViewModel, oldPos, oldAng, pos, ang)
	...
	BobCycleMultiplier =
		(BobCycleMultiplier > 1 and math.min(1 + ((BobCycleMultiplier - 1) * 0.2), 5) or BobCycleMultiplier)
...
function SWEP:DrawHUD()
	...
	if selectionDeg == 360 then
		selectionDeg = 0
	end

	if LocalPlayer():KeyDown(IN_RELOAD) then
		if animation < 1 then
			self.HUDAnimation = math.min(1, animation + FrameTime() * 2)
		end
		...
		if animation > 0 then
			self.HUDAnimation = math.max(0, animation - FrameTime() * 2)
		end
	...
	if animation ~= 0 then
		for i = 1, paintCount do
			...
			if animation == 1 then
				...
				local textW,
					textH = surface.GetTextSize(PaintName)
	...
	if IsValid(weapon.PaintGunViewModel) and self.Owner:GetViewEntity() == self.Owner then
		...
		local pos,
			ang = weapon.PaintGunViewModel:PreDraw()
		...
		if self.IsPickupEntity then
			weapon:DrawPickupEffects(weapon.PaintGunViewModel)
		end
		cam.End3D()
	end
...
if SERVER then
	...
	net.Receive(
		"TA_NW_PaintGun_SwitchPaint",
		function(len, ply)
			local mouse = net.ReadString()
			local paintType = net.ReadInt(8)

			if mouse == "first" then
				ply:GetActiveWeapon():SetNWInt("TA:firstPaintType", paintType)
			end
			if mouse == "second" then
				ply:GetActiveWeapon():SetNWInt("TA:secondPaintType", paintType)
			end
		end
	)
	...
	function SWEP:Think()
		local ply = self.Owner
		if not IsValid(self.PaintGunHoldEntity) or not self.PaintGunHoldEntity:IsPlayerHolding() then
			self:DropEntity()
		end
	...
	hook.Add(
		"AllowPlayerPickup",
		"TA:PaintGunDisablePickupbility",
		function(ply, ent)
			if IsValid(ent) and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "aperture_paintgun" then
				return false
			end
		end
	)
